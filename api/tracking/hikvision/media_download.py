...
<downloadRequest>
    ...
    def download_file(cls, auth_handler, cam_ip, file_uri, file_name):
        if os.path.exists(file_name):
            ...
            user_response = input(f"File {file_name} already exists. Do you want to append it? (y/n): ").lower().strip()
            if user_response != 'y':
                # User chose not to append, skip download
                Logger.get_logger().info(f"Skipped downloading {file_name}")
                return cls.FileDownloadingResult.ok()  # Return OK since the file is not being downloaded again
...
class LogPrinter:
    ...
    def download_tracks(tracks, auth_handler, cam_ip, content_type, channel):
        if len(tracks) == 0:
            Logger.get_logger().info('Start and End Datetime not acceptable: Found 0 files')
            return
...
def download_file_with_retry(auth_handler, cam_ip, track, content_type, channel):
    ...
    # Update: Create a directory structure based on date and channel
    date_directory = track.get_time_interval().start_time.strftime('%Y-%m-%d')
    channel_directory = f"{date_directory}/{channel}"
    
    directory_path = os.path.join(get_path_to_video_archive(cam_ip), channel_directory)

    # Update: Include only filename in the file path
    file_name = os.path.join(directory_path, f"{start_time_text}.{content_type}")

    create_directory_for(file_name)

    status = download_file(auth_handler, cam_ip, track.url_to_download(), file_name)
...
def do_work(camera_ip, start_datetime_str, use_utc_time, content_type, channel):
    logger = Logger.get_logger()
    try:
        ...
        end_datetime = start_datetime + timedelta(days=1)  # Set end time to 24 hours later

        utc_time_interval = TimeInterval(start_datetime, end_datetime, local_time_offset).to_utc()
...
Examples:
  ...
  %(prog)s -u 10.145.17.202 2020-04-15 00:30:00 2020-04-15 10:59:59
    ...
    parser.add_argument("START_TIME", help="start time of interval", default='00:00:00')
    parser.add_argument("END_DATE", help="end date of interval")
    parser.add_argument("END_TIME", help="end time of interval", default='23:59:59')
...
def main():
    ...
    if parameters:
        try:
            ...
            start_datetime_str = parameters.START_DATE + ' ' + '00:00:00'
            start_datetime = datetime.strptime(start_datetime_str, '%Y-%m-%d %H:%M:%S')

            # Set the end datetime to 24 hours after the start datetime
            ...
            if not is_download_completed(camera_ip, start_datetime_str, end_datetime_str, channel_cam):
                # Download media only if it's not completed
                do_work(camera_ip, start_datetime_str, end_datetime_str, content_type, channel_cam)

                # Mark the download as completed
                mark_download_completed(camera_ip, start_datetime_str, end_datetime_str, channel_cam)

            else:
                print("Download is already completed. No further action needed.")
...
def is_download_completed(camera_ip, start_datetime_str, end_datetime_str, channel):
    # Check if download is completed by looking at the status file
    status_file_path = f"{camera_ip}_{start_datetime_str}_{end_datetime_str}_{channel}_status.txt"
    
    return os.path.exists(status_file_path)

def mark_download_completed(camera_ip, start_datetime_str, end_datetime_str, channel):
    # Mark download as completed by creating a status file
    status_file_path = f"{camera_ip}_{start_datetime_str}_{end_datetime_str}_{channel}_status.txt"
